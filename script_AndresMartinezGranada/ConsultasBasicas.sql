...
SELECT transaccion_id, tipo_transaccion, monto
  FROM transacciones
  WHERE monto > 1000;
...
SELECT empleado_id, nombre, apellido, fecha_contratacion
  FROM empleados
  ORDER BY fecha_contratacion DESC
  LIMIT 5;

--7. Selecciona todas las transacciones realizadas entre el 1 de enero de 2023 y el 31 de diciembre de 2023.
SELECT transaccion_id, tipo_transaccion, monto, fecha_transaccion
  FROM transacciones
...
SELECT cuenta_id, numero_cuenta, tipo_cuenta
  FROM cuentas_bancarias
  WHERE tipo_cuenta IN ('ahorro','corriente','inversion');
